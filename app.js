...
function renderPokemons(data) {
  ...
  data.map((item) => {
    box.innerHTML += `
      <div class="relative w-[250px] bg-[#FFFF00] p-[30px] rounded-[10px] text-center">
        <p class="absolute top-[0] right-[0] bg-[#FF0000] rounded-tr-[10px] p-[5px] text-white">
          ${item.id}
        </p>
        <p class="font-bold text-[22px] pb-[10px]">${item.name}</p>
        <img class="w-[100px] mx-auto pb-[10px]" src=${item.img} alt=""/>
        <p class="bg-[#C4E4FF] rounded-[15px] p-[8px] w-fit mx-auto mb-[10px]">
          ${item.type.join(", ")}
        </p>
        <p class="font-bold">Candy count: ${
          item.candy_count ? item.candy_count : "100"
        }</p>
        <p class="text-[22px] py-[10px]">${item.weight}</p>
        <p class="text-[#80007C]">${item.weaknesses.join(", ")}</p>
        <p class="absolute bottom-[0] left-[0] bg-[#00FFFF] rounded-bl-[10px] p-[5px]">
          ${item.spawn_time}
        </p>
      </div>
    `;
...
function filterPokemons() {
  ...
  let filtered = pokemons.filter((pokemon) => {
    let filterName = pokemon.name.toLowerCase().includes(keyword);

    let filterType = selectedType === "" || pokemon.type.includes(selectedType);
...
searchinput.addEventListener("input", filterPokemons); //inputga nimadir yozilganda ishlaydi
typeSelect.addEventListener("change", filterPokemons); // Select tanlanganda ishlaydi
